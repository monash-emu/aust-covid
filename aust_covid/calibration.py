...
def get_priors(vacc_ext: bool, abbreviations: pd.Series, tex_doc: TexDoc) -> list:
    ...
    description = (
        "The priors used in any of the four analysis presented "
...
def get_targets(tex_doc: TexDoc, cross_ref: bool = True) -> list:
    ...
    death_targets = (
        load_who_death_data(tex_doc)[TARGETS_START_DATE:]
        .rolling(window=TARGETS_AVERAGE_WINDOW)
        .mean()
        .dropna()
    )
    ...
    description = (
        f"The composite daily case data were then smoothed using a {TARGETS_AVERAGE_WINDOW}-day moving average. "
    ...
    if cross_ref:
        ...
        tex_doc.add_line(description, "Targets", "Notifications")
    description = (
        f"These data were also smoothed using a {TARGETS_AVERAGE_WINDOW}-day moving average. "
    ...
    targets.append(
        est.CustomTarget(
            "seropos_ceiling",
            pd.Series([seropos_ceiling], index=[ceiling_date]),
            truncation_ceiling,
            model_key="adult_seropos_prop",
        )
    )
    return targets
