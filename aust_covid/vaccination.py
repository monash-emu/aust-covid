...
from .constants import IMMUNITY_LAG
...
def get_vacc_data_masks(
    ...
    masks = {
        "age 16+, 2+ doses": [
            c
            for c in df.columns
            if "Number of people fully vaccinated" in c
            and not any(s in c for s in [" - M - ", " - F - "])
        ],
        "age 16+, 3+ doses": [
            col
            ...
            and any(
                [
                    ...
                    for s in [
                        ...
                        "with more than 2 doses",
                    ]
                ]
            )
            and any([s in col for s in ["16", "18"]])
            and not any([s in col for s in ["increase", "Percentage", "Indigenous"]])
        ],
        "age 16+, 4+ doses": [
            col
            for col in df.columns
            ...
            and any(
                [
                    s in col
                    for s in [
                        "Winter Boosters number",
                        "who have received 4 doses",
                        "Fourth dose number",
                    ]
                ]
            )
            and not any([s in col for s in ["30", "65", "increase"]])
        ],
        "age 12-15, 2+ doses": [
            col
            for col in df.columns
            if "12-15" in col
            and any([s in col for s in ["National", "Age group"]])
            and any([s in col for s in ["2 doses", "fully vaccinated"]])
            and not any([s in col for s in ["Indigenous", "Population", "- F -", "- M -"]])
        ],
        "age 5-11, 2+ doses": [
            col
            for col in df.columns
...
def add_derived_data_to_vacc(
    ...
    df["adult booster"] = df.loc[:, masks["age 16+, 3+ doses"] + masks["age 16+, 4+ doses"]].sum(
        axis=1
    )
    ...
    df["prop remaining primary full"] = df["prop primary full"].diff() / (
        1.0 - df["prop primary full"]
    )
    df["prop remaining adult booster"] = df["prop adult booster"].diff() / (
        1.0 - df["prop adult booster"]
    )
...
def get_full_vacc_props(
    ...
    for full_vacc_mask in masks:
        age_str = (
            full_vacc_mask.replace("Age group - ", "")
            .replace("- Number of people fully vaccinated", "")
            .replace(" ", "")
        )
        full_df[full_vacc_mask] = (
            vacc_df[full_vacc_mask] / vacc_df[f"Age group - {age_str} - Population"]
        )
    return full_df
